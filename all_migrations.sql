...
DROP FUNCTION IF EXISTS wacrm.handle_new_user();
...
BEGIN
  IF TG_OP = 'DELETE' THEN
    PERFORM wacrm.recompute_broadcast_counts(OLD.broadcast_id);
    RETURN OLD;
  END IF;

  -- INSERT or UPDATE â€” only recompute when status changed (or on fresh insert)
  IF TG_OP = 'INSERT' OR OLD.status IS DISTINCT FROM NEW.status THEN
    PERFORM wacrm.recompute_broadcast_counts(NEW.broadcast_id);
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER SET search_path = wacrm, public, extensions;
...
FOR EACH ROW EXECUTE FUNCTION wacrm.broadcast_recipient_aggregate_trigger();
-- ============================================================
...
BEGIN
  FOR pol IN
    ...
    FROM pg_policies
    WHERE schemaname IN ('public', 'wacrm')
...
SELECT wacrm.merge_duplicate_contacts();
...
-- ============================================================
-- AUTO CREATE DEAL FOR NEW CONTACTS TRIGGER
-- ============================================================
...
-- ============================================================
-- SEED AUTOMATION TEMPLATES FOR PRESENTATIONS
-- ============================================================
